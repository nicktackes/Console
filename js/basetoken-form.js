...
/**
 * ext.ux.lc.form.BaseToken Extension Class for Ext 3.3 Library.  an abstract base class for token management forms
 *
 * @author  Nick Tackes
 *
 * @class ext.ux.lc.form.BaseToken
 * @extends Ext.form.FormPanel
 */
ext.ux.lc.form.BaseToken = Ext.extend(Ext.form.FormPanel, {
	...
	initComponent: function() {
		...
		ext.ux.lc.form.BaseToken.superclass.initComponent.call(this);
		...
		this.dbUserField = {
			...
			listeners:{
				'select':{fn: function(combo, value) {
						...
						if(tableNames) {
							var store = ext.ux.lc.UserTokenTableStores.getStore(combo.getValue());
							if(store) {
				...
				'change':{fn: function(combo, newValue, oldValue) {
						var tableNames = Ext.getCmp(combo.ownerCt.ownerCt.id + '_tableName');
						if(tableNames) {
							var store = ext.ux.lc.UserTokenTableStores.getStore(newValue);
							if(store) {
								tableNames.store = store;
							}
						}
			...
			buttons: [{
				...
				handler: function(btn, event) {
					...
					if (frm) {
						...
						if(!this.reswin) {
							this.reswin = new ext.ux.lc.TokenResultWindow({
								environ: this.environ,
...
// register xtype
Ext.reg('alvformbasetoken', ext.ux.lc.form.BaseToken);
