...
MyDesktop = new ext.ux.lc.TokenManagementApp ({
	init : function() {
		...
		var staticUrl = '../../examples/console/data/';
		this.environ = {
	...
	getStartConfig : function() {
		return {
			...
			},'-',
			{
				...
				tooltip: 'about this Lost Coast Studios product...',
				handler: function() {
					var desktop = this.getDesktop();
					var win = desktop.getWindow('about-win');
					if(!win) {
...
MyDesktop.WizardWindow = Ext.extend(Ext.app.Module, {
	...
	createWindow : function() {
		...
		ext.ux.lc.WizardItinaryManager.start('overview');
	}
});
				...
				items:[{
					...
					windowContents: function(scope) {
						...
						return {
                            ...
                            anchor: '-20',
                            environ: scope.app.environ,
                            url:'http://givesback.herokuapp.com/index.html'
                        };
					}
				}]
			}
