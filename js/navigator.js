 ...
 * @xtype alvnavigator
 */
ext.ux.lc.Navigator = Ext.extend(Ext.Viewport, {
	...
	initComponent: function() {
		...
		this.items.push({
			...
			listeners: {
				'tabchange': function(tabPanel, tab) {
					...
					/* Concept of a history combo in the south region of the navigator to jump to a particular location in history.
 					var historyStore = ext.ux.lc.HistoryDispatch.getStore();
 					if (historyStore) {
			...
			buttons: [{
				...
				//          selectOnFocus: true,
				//          xtype: 'combo',
				//          store: ext.ux.lc.HistoryDispatch.getStore(),
				//          allowBlank: true
				//      }, {
		...
		ext.ux.lc.Navigator.superclass.initComponent.call(this);
...
// register xtype
Ext.reg('alvnavigator', ext.ux.lc.Navigator);

// register history dispatch
ext.ux.lc.regh('alvnavigator', function(parts) {
