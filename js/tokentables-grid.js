...
/**
 * ext.ux.lc.grid.TokenizedTables Extension Class for Ext 3.3 Library
 *
 * @author  Nick Tackes
 * @version $Id: ext.ux.lc.grid.TokenizedTables.js
 *
 * @class ext.ux.lc.grid.TokenizedTables
 * @extends Ext.grid.EditorGridPanel
 */
ext.ux.lc.grid.TokenizedTables = Ext.extend(Ext.grid.GridPanel, {
	...
	initComponent: function() {
								...
								for (var i = 0; i < store.data.items.length; i++) {
									if (store.data.items[i].data && store.data.items[i].data.username == dbUser) {
										...
										if(dbPassword) {
											...
											//grid.stopEditing();
											ext.ux.lc.UserTokenTableStores.register(dbUser, dbPassword, true);
		...
		ext.ux.lc.grid.TokenizedTables.superclass.initComponent.call(this);

	}
});

// register xtype
Ext.reg('alvgridtokentables', ext.ux.lc.grid.TokenizedTables);
