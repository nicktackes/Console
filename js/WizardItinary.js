 ...
 * @author  Nick Tackes
 *
 * @class ext.ux.lc.WizardItinaryManager
 */
ext.ux.lc.WizardItinaryManager = function() {
	var wizards = {};
	var runningWizards = {};
	var tips = {};
	return {
		...
		start: function(name) {
			ext.ux.lc.WizardItinaryManager.next(name, 0);
		},
		next: function(name, index) {
			if(runningWizards[name]) {
				// if already started, then disregard
			} else if(ext.ux.lc.WizardItinaryManager.getTip(name, index)) {
				ext.ux.lc.WizardItinaryManager.getTip(name, index).show();
			} else {
				// kick off the wizard
				var wi = ext.ux.lc.WizardItinaryManager.get(name, index);
				var returns = {
				...
				if(wi && wi.length > index) {
						...
						new Ext.Toolbar({
							items: [{
								...
								handler: function(btn, event) {
									if(btn.wizardName) {
										var wi = ext.ux.lc.WizardItinaryManager.get(btn.wizardName);
										if(wi) {
											ext.ux.lc.WizardItinaryManager.removeTip(name, index);
										}
									}
								},
								scope: this
									...
									if(btn.wizardName) {
										...
										if(wi) {
											...
											if(wi.length > btn.wizardIndex+1) {
												ext.ux.lc.WizardItinaryManager.next(name,btn.wizardIndex-1);
											}
										}
									}
								},
						...
						listeners:{
							'hide': function(tooltip) {
								...
								if(tooltip.wizardName && tooltip.wizardIndex) {
									var wi = ext.ux.lc.WizardItinaryManager.get(tooltip.wizardName);
									if(wi) {
										ext.ux.lc.WizardItinaryManager.removeTip(tooltip.wizardName, tooltip.wizardIndex);
									}
								}
							}
						}
					...
					ext.ux.lc.WizardItinaryManager.addTip(name, index, tt);
		...
		stop: function(name) {
			var wi = ext.ux.lc.WizardItinaryManager.get(name);
		}
	};
}();
ext.ux.lc.WizardItinaryManager.register(
'overview',
[{
