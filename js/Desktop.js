...
Ext.Desktop = function(app) {
	...
	function setUserName(name) {
		taskbar.startMenu.setTitle(name);
	}
	...
	function markActive(win) {
		if (activeWindow && activeWindow != win) {
			// if there is an active window and its not maximized, and this window is not the active window,
			// then cascade this window so that it doesnt sit on top of last window.
			// check if its minimized because if so, we want to leave the window where it was.
			if(!activeWindow.maximized && !win.maximized && !win.minimized) {
				var x = activeWindow.getPosition()[0];
				var y = activeWindow.getPosition()[1];
	...
	if (shortcuts) {
		shortcuts.on('click', function(e, t) {
			...
			if (t) {
				e.stopEvent();
				var name = t.id.replace('-shortcut', '');
				var module = app.getModule(name);
				if (module) {
					module.createWindow();
				} else {
					// the module may have a subgroup.
					if(app.modules) {
						for(var y=0; y < app.modules.length; y++) {
							for(var x=0; x < app.modules[y].launcher.menu.items.length; x++) {
								if(app.modules[y].launcher.menu.items[x].code == name) {
									app.modules[y].createWindow(app.modules[y].launcher.menu.items[x]);
									return;
								}
							}
						}
