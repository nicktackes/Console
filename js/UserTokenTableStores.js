 ...
 * @author  Nick Tackes
 *
 * @class ext.ux.lc.UserTokenTableStores
 */
ext.ux.lc.UserTokenTableStores = function() {
	var stores = {};
	var environ;
	...
	var sampleValues = [];
	var me = ext.ux.lc.UserTokenTableStores;
	return {
		...
		register: function(username, password, overwrite) {
			...
			if(!stores[username] || overwrite) {
				if(!stores[username])
					stores[username] = ext.ux.lc.UserTokenTableStores.createStore();
				else if(overwrite) {
					// not needed
					//stores[username].removeAll(false);
				}
				// now load the store we just created
				ext.ux.lc.UserTokenTableStores.loadTokenTables(username, password);
				ext.ux.lc.UserTokenTableStores.loadTokenSamples();
			}
		},
		isSampleLoaded: function() {
			return ext.ux.lc.UserTokenTableStores.sampleDataLoaded;
		},
		getSampleValues: function() {
			return ext.ux.lc.UserTokenTableStores.sampleValues;
		},
		...
		loadTokenTables: function(username, password) {
			...
			Ext.Ajax.request({
				url: connectUrl,
				success: ext.ux.lc.UserTokenTableStores.dataLoadSuccess,
		...
		loadTokenSamples: function() {
			if(!ext.ux.lc.UserTokenTableStores.isSampleLoaded()) {
				// we will use the render event to initiate data load of the tokenized tables as well
				// as partial information (usernames) to populate the dbUser table with
				Ext.Ajax.request({
					url: environ.staticUrl + 'quicktestinputdata_valid.txt',
					success: ext.ux.lc.UserTokenTableStores.sampleDataLoadSuccess,
					failure: environ.communicator.dataLoadFailure,
					method: 'POST'
				});
			}
						...
						if(resultObj) {
							if(Ext.isArray(resultObj)) {
								for(var i = 0; i < resultObj.length; i++) {
									ext.ux.lc.UserTokenTableStores.processResultObject(resultData, resultObj[i], request);
								}
							} else {
								ext.ux.lc.UserTokenTableStores.processResultObject(resultData, resultObj, request);
							}
						}
						ext.ux.lc.UserTokenTableStores.getStore(request.params.dbUser).loadData({data:resultData});
					}
				} else {
					// result was retrieved successfully but the server report failure
					Ext.Msg.show({
		...
		sampleDataLoadSuccess: function(result, request) {
			...
			try {
				if (result && result.responseText) {
					ext.ux.lc.UserTokenTableStores.sampleValues = Ext.decode(result.responseText);
					ext.ux.lc.UserTokenTableStores.sampleDataLoaded = true;
				} else {
					// result was retrieved successfully but the server report failure
					Ext.Msg.show({
						title: 'Server Error',
		...
		requestTokens: function(payload, defaultParam, requestType, isStatic, staticResponseFile, paramName, serviceProvider, scope, successCallback, failureCallback) {
			...
			Ext.Ajax.request({
				...
				method: 'POST',
				success: (successCallback?successCallback:ext.ux.lc.UserTokenTableStores.trxResultLoadSuccess),
